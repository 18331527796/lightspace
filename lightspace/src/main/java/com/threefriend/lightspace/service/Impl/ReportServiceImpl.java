...
public class ReportServiceImpl implements ReportService{
	...
	public ResultVO pushReport(Map<String, String> params) {
		...
		int id,grade = 0,allstudent = 0,good = 0,bad= 0,avgGood= 0,avgMild= 0,avgModerate= 0,avgSerious= 0,leftbad= 0,
			nullstudent=0	,rightbad= 0,avgbad= 0,boy= 0,girl= 0,correct= 0,screeningbad= 0;
		String  type,schoolName="";
		...
		if("school".equals(type)) {
			...
			for (StudentMapper s : students) {
				...
				if(s.getVisionLeftStr()>=1.0d&&s.getVisionRightStr()>=1.0d&&(s.getVisionLeftStr()!=null||s.getVisionRightStr()!=null))good++;
				if(s.getVisionRightStr()<1.0d||s.getVisionLeftStr()<1.0d&&(s.getVisionLeftStr()!=null||s.getVisionRightStr()!=null))bad++;
				if(s.getVisionLeftStr()<1.0d&&s.getVisionRightStr()>=1.0d&&(s.getVisionLeftStr()!=null||s.getVisionRightStr()!=null))leftbad++;
				if(s.getVisionRightStr()<1.0d&&s.getVisionLeftStr()>=1.0d&&(s.getVisionLeftStr()!=null||s.getVisionRightStr()!=null))rightbad++;
				if(s.getVisionRightStr()<1.0d&&s.getVisionLeftStr()<1.0d&&(s.getVisionLeftStr()!=null||s.getVisionRightStr()!=null))avgbad++;
				if((s.getVisionRightStr()<1.0d||s.getVisionLeftStr()<1.0d&&s.getGender()==0&&(s.getVisionLeftStr()!=null||s.getVisionRightStr()!=null)))boy++;
				if((s.getVisionRightStr()<1.0d||s.getVisionLeftStr()<1.0d&&s.getGender()==1&&(s.getVisionLeftStr()!=null||s.getVisionRightStr()!=null)))girl++;
			}
			...
			avgGood = student_dao.classInAvgVision(allclass, VisionEnums.NORMAL.getType());
			avgMild = student_dao.classInAvgVision(allclass, VisionEnums.MILD.getType(),
					VisionEnums.NORMAL.getType());
			avgModerate = student_dao.classInAvgVision(allclass, VisionEnums.MODERATE.getType(),
					VisionEnums.MILD1.getType());
			avgSerious = student_dao.classInAvgVision(allclass, 0d, VisionEnums.SERIOUS.getType());
			correct = screening_wear_dao.findcountByClassId(allclass);
			screeningbad = screening_wear_dao.findcountByClassId(allclass,1.0d);
			...
			avgGood = student_dao.classInAvgVision(ids, VisionEnums.NORMAL.getType());
			avgMild = student_dao.classInAvgVision(ids, VisionEnums.MILD.getType(),
					VisionEnums.NORMAL.getType());
			avgModerate = student_dao.classInAvgVision(ids, VisionEnums.MODERATE.getType(),
					VisionEnums.MILD1.getType());
			avgSerious = student_dao.classInAvgVision(ids, 0d, VisionEnums.SERIOUS.getType());
			correct = screening_wear_dao.findcountByClassId(id);
			screeningbad = screening_wear_dao.findcountByClassId(ids,1.0d);
		}
        ...
        map.put("remark3", schoolName+"实际应检人数为:"+allstudent+"人   实检人数:"
        		+(allstudent-nullstudent)+ "人;\r\n"
        		+ "其中视力矫正人数为:"+correct+"人,占"+df.format(((float)correct/((float)(allstudent-nullstudent)))*100)+"%;\r\n"
        		+ "    视力未校正人数为:"+(allstudent-correct)+"人,占"+df.format(((float)((allstudent-nullstudent)-correct)/(allstudent-nullstudent))*100)+"%;\r\n"
        		+ "视力矫正中:\r\n"
