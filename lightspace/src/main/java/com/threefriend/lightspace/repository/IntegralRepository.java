...
public interface IntegralRepository extends JpaRepository<IntegralMapper, Integer>{
	...
	@Query(value="select count(1) from ( select SUM(integral) from integral_mapper where state = 1 and student_id in ?1 group by student_id HAVING SUM(integral) >= (select SUM(integral) from integral_mapper where student_id = ?2 and state = 1) ) a",nativeQuery = true)
	Long myRanking(List<Integer> studentIds, Integer studentId);
	
	@Query(value="select student_id from integral_mapper a where state = 1 and student_id in ?1 group by student_id order by SUM(integral) desc limit 10 ",nativeQuery = true)
	List<Integer> integtalRanking(List<Integer> studentIds);
	
	@Query(value="select student_id from integral_mapper a where state = 1 and student_id in ?1 group by student_id order by SUM(integral) desc limit ?2,?3 ",nativeQuery = true)
	List<Integer> integtalRanking(List<Integer> studentIds,int page , int size);
}
