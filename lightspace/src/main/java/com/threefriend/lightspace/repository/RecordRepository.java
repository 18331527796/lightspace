...
public interface RecordRepository extends JpaRepository<RecordMapper, Integer>{
	...
	@Query(value="SELECT count(1)" + 
			...
			" WHERE t2.id IS NULL AND t1.school_id = ?1 and t1.vision_left_str > ?2",nativeQuery = true)
	...
	@Query(value="SELECT count(1) FROM record_mapper t1" + 
			" LEFT JOIN record_mapper t2 ON t1.student_id = t2.student_id AND t1.gen_time < t2.gen_time" + 
			" WHERE t2.id IS NULL AND t1.school_id = ?1 and t1.vision_left_str BETWEEN ?2 and ?3",nativeQuery = true)
	int countTopBySchoolIdAndVisionLeftBetweenOrderByStudentId(Integer schoolId,Double min,Double max);
	//学校 右眼 大于
	@Query(value="SELECT count(1)" + 
			" FROM record_mapper t1" + 
			" LEFT JOIN record_mapper t2 ON t1.student_id = t2.student_id AND t1.gen_time < t2.gen_time " + 
			" WHERE t2.id IS NULL AND t1.school_id = ?1 and t1.vision_right_str > ?2",nativeQuery = true)
	int countTopBySchoolIdAndVisionRightGreaterThanOrderByStudentId(Integer schoolId,Double min);
	//学校 右眼区间
	@Query(value="SELECT count(1) FROM record_mapper t1" + 
			" LEFT JOIN record_mapper t2 ON t1.student_id = t2.student_id AND t1.gen_time < t2.gen_time" + 
			" WHERE t2.id IS NULL AND t1.school_id = ?1 and t1.vision_right_str BETWEEN ?2 and ?3",nativeQuery = true)
	int countTopBySchoolIdAndVisionRightBetweenOrderByStudentId(Integer schoolId,Double min,Double max);
	//学校 双眼 区间
	@Query(value="SELECT count(1) FROM record_mapper t1" + 
			" LEFT JOIN record_mapper t2 ON t1.student_id = t2.student_id AND t1.gen_time < t2.gen_time" + 
			" WHERE t2.id IS NULL AND t1.school_id = ?1 and (t1.vision_left_str + t1.vision_right_str)/2 BETWEEN ?2 and ?3",nativeQuery = true)
	int schoolAvgVision(Integer school,Double min,Double max);
	//学校 双眼 大于
	@Query(value="SELECT count(1)" + 
			" FROM record_mapper t1" + 
			" LEFT JOIN record_mapper t2 ON t1.student_id = t2.student_id AND t1.gen_time < t2.gen_time " + 
			" WHERE t2.id IS NULL AND t1.school_id = ?1 and (t1.vision_left_str + t1.vision_right_str)/2 > ?2",nativeQuery = true)
	int schoolAvgVision(Integer school,Double min);
	
	@Query(value="SELECT count(1)" + 
			" FROM record_mapper t1" + 
			" LEFT JOIN record_mapper t2 ON t1.student_id = t2.student_id AND t1.gen_time < t2.gen_time " + 
			" WHERE t2.id IS NULL AND t1.classes_id IN ?1 and t1.vision_left_str BETWEEN ?2 and ?3",nativeQuery = true)
	int countTopByClassesIdInAndVisionLeftBetweenOrderByStudentId(List<Integer> ClassId,Double min,Double max);
	
	@Query(value="SELECT count(1)" + 
			" FROM record_mapper t1" + 
			" LEFT JOIN record_mapper t2 ON t1.student_id = t2.student_id AND t1.gen_time < t2.gen_time " + 
			" WHERE t2.id IS NULL AND t1.classes_id IN ?1 and t1.vision_left_str > ?2",nativeQuery = true)
	int countTopByClassesIdInAndVisionLeftGreaterThanOrderByStudentId(List<Integer> ClassId,Double min);
	
	@Query(value="SELECT count(1)" + 
			" FROM record_mapper t1" + 
			" LEFT JOIN record_mapper t2 ON t1.student_id = t2.student_id AND t1.gen_time < t2.gen_time " + 
			" WHERE t2.id IS NULL AND t1.classes_id IN ?1 and t1.vision_right_str > ?2",nativeQuery = true)
	int countTopByClassesIdInAndVisionRightGreaterThanOrderByStudentId(List<Integer> ClassId,Double min);
	
	@Query(value="SELECT count(1)" + 
			" FROM record_mapper t1" + 
			" LEFT JOIN record_mapper t2 ON t1.student_id = t2.student_id AND t1.gen_time < t2.gen_time " + 
			" WHERE t2.id IS NULL AND t1.classes_id IN ?1 and t1.vision_right_str BETWEEN ?2 and ?3",nativeQuery = true)
	int countTopByClassesIdInAndVisionRightBetweenOrderByStudentId(List<Integer> ClassId,Double min,Double max);
	@Query(value="SELECT count(1)" + 
			" FROM record_mapper t1" + 
			" LEFT JOIN record_mapper t2 ON t1.student_id = t2.student_id AND t1.gen_time < t2.gen_time " + 
			" WHERE t2.id IS NULL AND t1.classes_id IN ?1 and (t1.vision_left_str + t1.vision_right_str)/2 BETWEEN ?2 and ?3",nativeQuery = true)
	int classInAvgVision(List<Integer> ClassId,Double min,Double max);
	
	@Query(value="SELECT count(1)" + 
			" FROM record_mapper t1" + 
			" LEFT JOIN record_mapper t2 ON t1.student_id = t2.student_id AND t1.gen_time < t2.gen_time " + 
			" WHERE t2.id IS NULL AND t1.classes_id IN ?1 and (t1.vision_left_str + t1.vision_right_str)/2 > ?2",nativeQuery = true)
